...
import PosturalChainAnalyzer from "./components/PosturalChainAnalyzer";
...
function App() {
    ...
    try {
      ...
      if (evaluacionesGuardadas.length > 10) {
        ...
        localStorage.setItem(
          "evaluaciones",
          JSON.stringify(evaluacionesRecientes)
        );
        setEvaluaciones(evaluacionesRecientes);

        alert(
          `Se eliminaron ${
            evaluacionesGuardadas.length - 10
          } evaluaciones antiguas para liberar espacio.`
        );
        return true;
      }
      return false;
  ...
  const compressAIAnalysis = async (
    analysisData,
    maxWidth = 300,
    quality = 0.4
  ) => {
    try {
      ...
      if (analysisData.analysisImage) {
        compressed.analysisImage = await compressImage(
          analysisData.analysisImage,
          maxWidth,
          quality
        );
      }
  ...
  const guardarEnLocalStorage = async () => {
    ...
    if (!patientData.nombre || patientData.nombre.trim() === "") {
      alert(
        "Por favor, ingrese al menos el nombre del paciente antes de guardar."
      );
      return;
    }
    ...
    if (storageInfo.percentage > 85) {
      ...
      if (shouldClean) {
        ...
        if (!cleaned) {
          alert(
            "No hay evaluaciones suficientes para limpiar. Considere exportar datos y empezar de nuevo."
          );
          return;
        }
      } else {
        alert(
          "Para guardar esta evaluación, necesita liberar espacio eliminando evaluaciones antiguas."
        );
        return;
      }
    }
    ...
    try {
      ...
      const originalButton = document.querySelector(
        'button[onclick*="guardarEnLocalStorage"]'
      );
      if (originalButton) {
        originalButton.disabled = true;
        originalButton.innerHTML =
          '<i class="fas fa-spinner fa-spin me-2"></i>Optimizando datos...';
      }
      ...
      for (const [viewName, analysisData] of Object.entries(
        aiAnalysisResults
      )) {
        if (analysisData && analysisData.analysisImage) {
          try {
            console.log(`Comprimiendo análisis IA: ${viewName}`);
            compressedAIAnalysis[viewName] = await compressAIAnalysis(
              analysisData
            );
          } catch (error) {
      ...
      const nuevaEvaluacion = {
        ...
        ...(patientData.antecedentesMedicos && {
          antecedentesMedicos: patientData.antecedentesMedicos,
        }),
        ...(patientData.medicamentos && {
          medicamentos: patientData.medicamentos,
        }),
        ...(patientData.actividad_fisica && {
        ...
        // IMC calculado
        imc:
          patientData.peso && patientData.altura
      ...
      console.log(
        `Tamaño de la evaluación optimizada: ${sizeInMB.toFixed(2)} MB`
      );

      if (sizeInMB > 2) {
        alert(
          "La evaluación sigue siendo muy grande. Intente con menos imágenes o reduzca la calidad."
        );
        return;
      }
      ...
      console.log(
        `Tamaño total del almacenamiento: ${totalSizeInMB.toFixed(2)} MB`
      );
      ...
      if (totalSizeInMB > 4) {
        ...
        if (!cleaned) {
          alert(
            "Almacenamiento lleno. Se necesita eliminar evaluaciones antiguas manualmente."
          );
          return;
        }
        ...
        localStorage.setItem(
          "evaluaciones",
          JSON.stringify(actualizadasLimpias)
        );
      ...
      const mensaje =
        `✅ Evaluación de ${patientData.nombre} guardada exitosamente.\n` +
      ...
      if (error.name === "QuotaExceededError") {
        alert(
          '❌ Almacenamiento lleno. Use "Limpiar Almacenamiento" y intente nuevamente.'
        );
      } else {
      ...
      if (originalButton) {
        ...
        originalButton.innerHTML =
          '<i class="fas fa-database me-2"></i>Guardar Evaluación';
      }
    }
  };
  ...
  const limpiarAlmacenamiento = () => {
    const mensaje =
      `Uso actual del almacenamiento: ${storageInfo.percentage.toFixed(
        1
      )}%\n\n` +
    ...
    switch (opcion) {
      ...
      case "2":
        if (
          window.confirm(
            "⚠️ ¿Está seguro? Esto eliminará TODAS las evaluaciones guardadas."
          )
        ) {
  ...
  const exportarTodosLosDatos = () => {
    try {
      ...
      alert(
        "✅ Backup completo exportado. Ahora puede limpiar el almacenamiento si lo desea."
      );
    } catch (error) {
  ...
  const handleEdit = (updatedEval) => {
    try {
      const actualizadas = evaluaciones.map((ev) =>
        ev.id === updatedEval.id ? updatedEval : ev
      );
      localStorage.setItem("evaluaciones", JSON.stringify(actualizadas));
      setEvaluaciones(actualizadas);
      setSelectedEvaluation(null);
    ...
    <div className="alert alert-info mb-3">
      <div className="d-flex justify-content-between align-items-center">
        <div>
          ...
          <strong>Almacenamiento:</strong> {storageInfo.percentage.toFixed(1)}%
          usado
        </div>
        <div>
    ...
    <div className="container-fluid py-4" style={{ maxWidth: "1400px" }}>
      ...
      <div className="text-center mb-5">
        ...
        <p className="lead text-muted">
          Análisis computarizado con IA para apoyo a diagnostico de problemas
          posturales
        </p>
      </div>
      ...
      />
      <PosturalChainAnalyzer deviations={deviations} />
              ...
              <div className="row">
                ...
                <div className="col-md-2 mb-3">
                  <ExportPDFButton
                    images={images}
                    patientData={patientData}
                    deviations={deviations} // ← Debe estar esta línea
                  />
                </div>
                <div className="col-md-2 mb-3">
                ...
                <div className="col-md-2 mb-3">
                  <div className="text-center">
                    ...
                    <div className="mt-2">
                      <small className="text-muted">Reiniciar formulario</small>
                    </div>
                  </div>
                </div>
                <div className="col-md-2 mb-3">
                  <div className="text-center">
                    ...
                    <div className="mt-2">
                      <small className="text-muted">Exportar todo</small>
                    </div>
                  </div>
                </div>
                <div className="col-md-2 mb-3">
                  <div className="text-center">
                    ...
                    <div className="mt-2">
                      <small className="text-muted">Liberar espacio</small>
                    </div>
                  </div>
                </div>
              </div>
                    ...
                    <div className="alert alert-info">
                      <div className="row">
                        <div className="col-md-4">
                          ...
                          <strong>Imágenes originales:</strong>{" "}
                          {Object.keys(images).length}
                        </div>
                        <div className="col-md-4">
                          <i className="fas fa-robot me-2"></i>
                          <strong>Análisis de IA:</strong>{" "}
                          {Object.keys(aiAnalysisResults).length}
                        </div>
                        <div className="col-md-4">
                          <i className="fas fa-compress me-2"></i>
                          <strong>Compresión:</strong> Optimizada
                          automáticamente
                        </div>
                      </div>
                    </div>
          ...
          <div className="row mb-3">
            ...
            <div className="col-md-4">
              ...
              >
                ...
                <option value="Retroversión de cadera">
                  Retroversión de cadera
                </option>
                <option value="Anteversión de cadera">
                  Anteversión de cadera
                </option>
                <option value="Hipercifosis torácica">
                  Hipercifosis torácica
                </option>
                <option value="Hiperlordosis lumbar">
                  Hiperlordosis lumbar
                </option>
                <option value="Rectificación cervical">
                  Rectificación cervical
                </option>
                <option value="Rectificación lumbar">
                  Rectificación lumbar
                </option>
                <option value="Escoliosis funcional">
                ...
                <option value="Rotación interna de hombros">
                  Rotación interna de hombros
                </option>
                <option value="Elevación unilateral de hombro">
                  Elevación unilateral de hombro
                </option>
                <option value="Protracción de cabeza">
                  Protracción de cabeza
                </option>
                <option value="Protrusión abdominal">
          ...
          <div className="row">
            ...
            <div className="col-md-6 text-end">
              ...
              >
                <i
                  className={`fas fa-${
                    showAnalysis ? "eye-slash" : "robot"
                  } me-2`}
                ></i>
                {showAnalysis
                  ? "Ocultar Análisis IA"
                  : "Ver Análisis Postural (IA)"}
              </button>
            </div>
          </div>
        </div>
